...
# TODO: Look if this is slowing things down
class SequenceSegment:
    def __init__(self, sequence_id: bytes, data: bytes = b''):
        self.id = sequence_id
        self.data = data
...
def sequence_segments(
        ...
        # Otherwise the line we are on is sequence data
            ...
            while len(working_sequence_buffer) >= sequence_length:
                yield SequenceSegment(
                    current_sequence_id,
                    working_sequence_buffer[:sequence_length])
    ...
    if len(working_sequence_buffer) > sequence_lookahead_length:
        yield SequenceSegment(current_sequence_id, working_sequence_buffer)
